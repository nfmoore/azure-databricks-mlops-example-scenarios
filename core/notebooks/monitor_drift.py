...
dbutils.widgets.text("reference_dataset_table", "hive_metastore.default.employee_attrition_curated")
dbutils.widgets.text("target_dataset_table", "hive_metastore.default.employee_attrition_inference")
...
def load_data():
    ...
    # load datasets
    reference_df = spark.read.table(dbutils.widgets.get("reference_dataset_table")).toPandas()
    target_df = spark.read.table(dbutils.widgets.get("target_dataset_table")).toPandas()
...
def evaluate_drift(reference_df, target_df):
    ...
    data_drift_report.run(reference_data=reference_df, current_data=target_df, column_mapping=column_mapping)
    ...
    for feature in column_mapping.numerical_features + column_mapping.categorical_features:
        drift_metrics.append((feature, report["metrics"][1]["result"]["drift_by_columns"][feature]["drift_score"]))
